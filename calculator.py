...
for i in range(10):
    if i == 0:
        button = Button(window, text=str(i), command=lambda: type_number(button), width=10, height=5).grid(row=4, column=1)
    else:
        index = i-1
        button = Button(window, text=str(i), command=lambda: type_number(button), width=10, height=5).grid(row=math.floor(index/3)+1, column=index%3)
for i in range(4):
    operations = ['+', '-', '*', '/']
